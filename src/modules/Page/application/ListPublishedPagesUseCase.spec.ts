...
describe('ListPublishedPagesUseCase', () => {
  ...
  it('deja fuera las páginas despublicadas: no pueden llegar a buscadores', async () => {
    const repository = buildRepository([
      new Page('home', 'Inicio', null, [], 1, true),
      new Page('borrador', 'Borrador', null, [], 2, false),
    ]);

    const pages = await new ListPublishedPagesUseCase(repository).execute(7);
  ...
  it('consulta solo las páginas del tenant pedido', async () => {
    ...
    await new ListPublishedPagesUseCase(repository).execute(7);

    expect(repository.findAllByTenant).toHaveBeenCalledWith(7);
  });
  ...
  it('entrega la fecha de modificación en ISO, para el lastModified del sitemap', async () => {
    ...
    const repository = buildRepository([
      new Page('home', 'Inicio', null, [], 1, true, updatedAt),
    ]);
  ...
  it('no lleva secciones: el sitemap solo necesita la dirección y el título', async () => {
    const repository = buildRepository([new Page('home', 'Inicio', 'Desc', [], 1, true)]);
